...
# So long as backfill is on, this will automatically backfill 0s if the conditions are met

check_seen_multi <- function(dat, var_list, condition = "TRUE", backfill = TRUE) {
  ...
  for(i in 1:length(var_list)) {
    ...
    inconsistent1 <- !is.na(match(TRUE, soft_errors1)) & !is.na(match(TRUE, soft_errors2)) #did some cases have na when they should have soon, and 0 when they should not have seen?
