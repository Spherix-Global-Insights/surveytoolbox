...
# Searches through the column names of the data to return variables that match the given criteria
...
find_var_name <- function(dat, keyword, omit = NULL, all_keyword = TRUE, all_omit = FALSE) {
  ...
  for(i in 1:length(keyword)) {
    ...
    new_match <- str_detect(colnames(dat), keyword[i])
  ...
  if(is.null(omit)) {
    ...
    for(i in 1:length(omit)) {
      ...
      new_omit <- str_detect(colnames(dat), omit[i])
